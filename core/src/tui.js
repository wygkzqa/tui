...
;(function () {
      ...
      return (
          <form id={this.props.formId} className={className} encType="multipart/form-data" method={method}>
            ...
            <div className="form-group">
              <a href="javascript:;" className="col-sm-2" onClick={this.props.backList}><i
                  className="fa fa-arrow-left"></i>&nbsp;返回列表</a>
            </div>
            <hr/>
            {
  ...
  var EntityHandle = React.createClass({
    ...
    render: function () {
      ...
      if (keyValue === undefined) {
        createOrUpdateDOM = <button type="button" className="ebtn ebtn-success ebtn-rounded tui-mr5"
                                    onClick={this.props.create}>添加</button>;
        ...
        if (update.condition && typeof update.condition === 'function') {
          ...
          if (update.condition(entityData) === 'success') {
            createOrUpdateDOM = <button type="button" className="ebtn ebtn-success ebtn-rounded tui-mr5"
                                        onClick={this.props.create}>保存</button>;
          }
        } else {
          createOrUpdateDOM = <button type="button" className="ebtn ebtn-success ebtn-rounded tui-mr5"
                                      onClick={this.props.create}>保存</button>;
      ...
      for (var i = 0; i < custom.length; i++) {
        ...
        customButtons.push(<button type="button" className={'tui-mr5 ' + _custom.className} onClick={this.customHandle}
                                   data-customid={i}>{_custom.text}</button>);
      }
      ...
      return (
          <div className="form-group">
            <div className="col-sm-4 col-sm-offset-2">
              {createOrUpdateDOM}
              <button type="button" className="ebtn ebtn-default ebtn-rounded tui-mr5"
                      onClick={this.props.backList}>取消
              </button>
              {customButtons}
    ...
    render: function () {
      ...
      if (this.props.type !== 'file' && this.props.keyValue !== '') {
        if (this.props.type === 'date') {
          return (
              <input type="text" className="form-control" name={field.field} ref={field.field}
                     placeholder={field.placeholder || field.text} value={this.state.value} onFocus={this.dateFocus}
                     onChange={this.entityTextChangeHandle} disabled={field.disabled || false}
                     readOnly={field.readOnly || false}/>
          );
        }

        return (
            <input type={this.props.type} className="form-control" name={field.field} ref={field.field}
                   placeholder={field.placeholder || field.text} value={this.state.value}
                   onChange={this.entityTextChangeHandle} disabled={field.disabled || false}
                   readOnly={field.readOnly || false}/>
        );
      } else {
        return (
            <input type={this.props.type} className="form-control" name={field.field} ref={field.field}
                   placeholder={field.placeholder || field.text} defaultValue=""/>
        );
      }
    }
  });
  ...
  var EntitySelect = React.createClass({
    ...
    render: function () {
      ...
      if (field.options instanceof Array) {
        return (
            <select ref={field.field} name={field.field} className="form-control" value={this.state.value}
                    onChange={this.entitySelectChangeHandle}>
              {
                    ...
                    onChange={this.entitySelectChangeHandle}>
              {
                this.state.options.rows.map(function (op, key) {
                  return <option key={key}
                                 value={op[this.state.options.valueField]}>{op[this.state.options.textField]}</option>;
                }.bind(this))
              }
            </select>
        );
          ...
          <p>
            {
              field.options.map(function (op, key) {
                ...
                if (this.props.keyValue !== '') {
                  radioDOM =
                      <input checked={this.state.value == op.value} type="radio" ref={field.field} name={field.field}
                             value={op.value} onChange={this.entityRadioChangeHandle}/>;
                } else {
      ...
      if (this.props.keyValue) {
        return (
            <textarea rows="5" className="form-control" name={field.field} ref={field.field} placeholder={field.text}
                      value={this.state.value} onChange={this.entityTextareaChangeHandle}
                      disabled={field.disabled || false} readOnly={field.readOnly || false}/>
        );
      } else {
        return (
            <textarea rows="5" className="form-control" name={field.field} ref={field.field} placeholder={field.text}
  ...
  var EntityEditor = React.createClass({
    ...
    render: function () {
      ...
      if (this.props.keyValue === '') {
        return (
            <textarea id={field.field} rows="5" className="form-control" name={field.field} ref={field.field}
  ...
  var SearchBar = React.createClass({
    ...
    render: function () {
      ...
      if (this.props.hasAdd) {
        addBtn = <button type="button" className="ebtn ebtn-success ebtn-rounded" onClick={this.props.goCreate}><i
            className="fa fa-plus"></i>&nbsp;添加</button>;
      }
      ...
      if (this.props.searchbar.length === 0 && this.props.hasAdd) {
        ...
        return (
            <div className="tui-searchbar">
                  ...
                  if (typeof obj === 'object') {
                    ...
                    switch (type) {
                      ...
                      case 'text':
                        searchbarDOM = (
                            <div className="form-group" key={key}>
                              ...
                              <input id={obj.field} name={obj.field} type="text" ref={obj.field}
                                     className="form-control" placeholder={obj.placeholder || obj.text}/>&nbsp;
                            </div>
                        );
                        break;
                        // 下拉框
                          ...
                          $.ajax({
                            ...
                            success: function (result) {
                              ...
                              searchbarDOM = (
                                  <div className="form-group" key={key}>
                                    <label htmlFor={obj.field}>{obj.text}</label>
                                    <select id={obj.field} name={obj.field}
                                            className="form-control">{optionsArr}</select>&nbsp;
                                  </div>
                              );
                            },
                            error: function (xhr, error, obj) {
                      ...
                      case 'radio':
                        var options = [];
                        for (var i = 0; i < obj.options.length; i++) {
                          options.push(<label><input type="radio" name={obj.field}
                                                     value={obj.options[i].value}/>{obj.options[i].text}</label>);
                        }
                      ...
                      case 'date':
                        searchbarDOM = (
                            <div className="form-group" key={key}>
                              <label htmlFor={obj.field}>{obj.text}</label>
                              <input id={obj.field} name={obj.field} type="text" ref={obj.field}
                                     className="form-control" onFocus={this.dateFocus} onBlur={this.dateBlur}
                                     placeholder={obj.placeholder || obj.text}/>&nbsp;
                            </div>
                        );
                        break;
                      default:
              ...
              <div className="form-group">
                &nbsp;
                <button type="button" className="ebtn ebtn-primary ebtn-rounded" onClick={this.props._pagingClick}
                        data-page="1"><i className="fa fa-search"></i>&nbsp;查询
                </button>
                &nbsp;
          ...
          <div className="tui-toolbar">
            {
              this.props.toolbar.map(function (tb, key) {
                ...
                switch (type) {
                  case 'button':
                    tbDOM =
                        <button className={(tb.className ? tb.className : 'btn btn-primary') + ' tui-mr5'} type="button"
                                onClick={tb.handle}>{tb.text}</button>;
                    break;
                  case 'checkbox':
                    var id = tb.id || '',
                        name = tb.name || '',
                    ...
                    tbDOM = <div><label htmlFor={id} className="tui-mr5">{tb.text}</label><input type="checkbox" id={id}
                                                                                                 name={name} style={{
                      'width': '15',
                      'height': '15'
                    }} defaultChecked={tb.checked}/></div>;
                    break;
                  default:
                    tbDOM =
                        <button className={(tb.className ? tb.className : 'btn btn-primary') + ' tui-mr5'} type="button"
  ...
  var TableRow = React.createClass({
    ...
    getParentStatusDOM: function (status) {
      return status === 'hide' ? (
          <span><i className="fa fa-angle-right tui-mr5"></i><i className="fa fa-folder-open-o tui-mr5"></i></span>) : (
          <span><i className="fa fa-angle-down tui-mr5"></i><i className="fa fa-folder-open tui-mr5"></i></span>);
    },
    /**
     * 删除事件
     */
    ...
    render: function () {
      ...
      if (rowHandles) {
        ...
        if (typeof rowHandles.find === 'function') {
          rowHandlesDOM.push(<a href="javascript:;" className="tui-mr5" title="详情" onClick={this.findHandle}><i
        ...
        if (rowHandles.custom && customHandles instanceof Array) {
          for (var i = 0; i < customHandles.length; i++) {
            ...
            customHandlesDOM.push(<li><a href="javascript:;" onClick={this.customHandles} data-customid={i}><i
                className={custom.iconClass ? custom.iconClass : ''}></i>&nbsp;{custom.text}</a></li>);
          }
        }

        // 删除
        if (typeof rowHandles.delete === 'function') {
          customHandlesDOM.push(<li><a href="javascript:;" className="tui-mr5" title="删除" onClick={this.deleteHandle}
                                       style={{'marginRight': '5px'}}><i
              className="fa fa-trash-o fa-fw"></i>&nbsp;删除</a></li>);
        }

        if (customHandlesDOM.length > 0) {
          rowHandlesDOM.push(<div className="btn-group">
            <a href="javascript:;" className="dropdown-toggle tui-dropdown-menu" data-toggle="dropdown"
              ...
              this.props.columns.map(function (col, key) {
                ...
                if (!col.handle && col.field.indexOf('.') === -1 && !col.checkbox) {
                  ...
                  // 是否有chexkbox
                    ...
                    return (<td key={key}><input type="checkbox" name={col.field} value={row[col.field]}
                                                 checked={row.isCheck} onChange={this.handlerCheck}/></td>);
                  }
                }
              }.bind(this))
            }
    ...
    render: function () {
      ...
      if (pageCount > maxPageCount) {
        ...
        if (currentPage - 5 >= 0) {
          ...
          pagesDOM.push(<li key={1}><a href="javascript:;" onClick={this.props._pagingClick} data-page={1}>{1}...</a>
          </li>);
          end++;
        } else {
      ...
      if (pageCount - currentPage > 3) {
        pagesDOM.push(<li key={pageCount}><a href="javascript:;" onClick={this.props._pagingClick}
                                             data-page={pageCount}>...{pageCount}</a></li>);
      }
      ...
      return (
          <nav>
            <ul className="pager">
              ...
              <li>
                <a href="javascript:;" style={{'padding': '2px 7px'}}
                   onClick={currentPage === 1 || currentPage - 1 === 0 ? '' : this.props._pagingClick}
              ...
              <li>
                <a href="javascript:;" style={{'padding': '2px 7px'}}
                   onClick={currentPage === pageCount || currentPage + 1 > pageCount ? '' : this.props._pagingClick}
                   data-page={currentPage + 1}>
  ...
  var ModalComp = React.createClass({
    ...
    render: function () {
      ...
      if (typeof this.props.confirm === 'function') {
        confirmBtn =
            <button type="button" className="ebtn ebtn-success ebtn-rounded" onClick={this.confirm}>确定</button>;
      }
            ...
            <div className={'modal-dialog ' + this.props.type} role="document">
              <div className="modal-content">
                ...
                <div className="modal-footer">
                  <button type="button" className="ebtn ebtn-default ebtn-rounded" data-dismiss="modal">取消</button>
                  &nbsp;{confirmBtn}
                </div>
              </div>
            </div>
  ...
  var Loading = function (obj) {
    ...
    ReactDOM.render(React.createElement(LoadingModal, {
      className: obj && obj.className ? obj.className : 'show',
      text: obj && obj.text ? obj.text : '正在加载'
    }), dom);
  }
